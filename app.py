...
# Sayfa Ayarları
st.set_page_config(page_title="Akademik Ders Programı V18.0", layout="wide")

st.title("🎓 Akademik Ders Programı Dağıtıcı (V18.0 - Hafta İçi Modu)")
st.success("✅ Cumartesi günü sistemden tamamen çıkarıldı. Sadece Pazartesi-Cuma arası planlama yapılır.")

# --- PARAMETRELER ---
MAX_SURE = 180            
DERSLIK_SAYISI = 200       

# CEZA PUANLARI (Çözüm Garantili - Soft Constraints)
...
# --- ŞABLON OLUŞTURMA (CUMARTESİ DERSLERİ TEMİZLENDİ) ---
def sablon_olustur():
    data = [
        # --- TURİZM ---
        ...
        {"DersKodu": "YDB 3809", "Bolum": "Turizm İşletmeciliği", "Sinif": 3, "HocaAdi": "Öğr.Gör.İsmail Zeki DİKİCİ", "OrtakDersID": "", "KidemPuani": 1},
        {"DersKodu": "YDB 3917", "Bolum": "Turizm İşletmeciliği", "Sinif": 3, "HocaAdi": "Öğr. Gör. Ümit KONAÇ", "OrtakDersID": "", "KidemPuani": 1},
        {"DersKodu": "YDB 4821", "Bolum": "Turizm İşletmeciliği", "Sinif": 4, "HocaAdi": "Öğr.Gör.İsmail Zeki DİKİCİ", "OrtakDersID": "", "KidemPuani": 1},
        {"DersKodu": "YDB 4907", "Bolum": "Turizm İşletmeciliği", "Sinif": 4, "HocaAdi": "Öğr. Gör. Ümit KONAÇ", "OrtakDersID": "", "KidemPuani": 1},
    ...
    aciklamalar = [
        ...
        "ÖNEMLİ: CUMARTESİ GÜNÜ ÇIKARILMIŞTIR.",
...
def programi_coz(df_veri):
    ...
    gunler = ['Pazartesi', 'Sali', 'Carsamba', 'Persembe', 'Cuma'] # Cumartesi çıkarıldı
    ...
    # 3. Bölüm/Sınıf Çakışması (SOFT - CEZALI)
...
with col1:
    ...
    st.download_button(
        label="📥 Tam Verili Şablon İndir (V18.0)",
        data=sablon_olustur(),
        file_name="Ders_Yukleri_Tam_V18.xlsx",
...
if uploaded_file is not None:
    if st.button("Programı Dağıt"):
        with st.spinner('Program zorla oluşturuluyor... (Kurallar ihlal edilebilir)'):
            try:
                ...
                if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                    ...
                    st.subheader("⚠️ Çakışma ve İhlal Raporu")
                    ...
                    for h in hoca_listesi:
                        for g in gunler:
                            for s in seanslar:
                                ...
                                if count > 1:
                                    st.error(f"HOCA ÇAKIŞMASI: {h} -> {g} {s}: {count} ders aynı anda!")
                    ...
                    st.download_button(
                        ...
                        data=processed_data,
                        file_name="Final_Program_V18.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
                    
                else:
                    st.error("❌ Beklenmedik durum.")
